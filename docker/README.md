...
docker build -t myimage:v1 .                    # build an image from a Dockerfile in the current directory
docker pull myimage:v1                            # download an image from a registry without running it
docker push myregistry/myimage:v1                   # upload an image to a registry
docker tag myimage:v1 myregistry/myimage:v1           # add a new tag to an existing local image, e.g. before pushing
docker rm mycontainer                                   # remove a stopped container
docker rmi myimage:v1                                     # remove a local image
docker cp mycontainer:/app/log.txt ./log.txt                # copy a file out of a container without exec-ing in
docker top mycontainer                                         # processes running inside a container, from the host's view
...
docker run --rm -it --user 0 myimage sh                # drop into a container as root even if the image sets a non-root default user, for debugging permission issues
docker logs --details mycontainer                        # include extra attributes in log output (labels etc.), not just the log line itself
```
