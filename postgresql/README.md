...
```bash
pg_dump -h <host> -U <username> -d <database> -Fp > backup.sql          # plain SQL, human-readable, restore with psql -f
...
pg_dump -t <schema>.<table> -h <host> -U <username> -d <database> > table.sql       # a single table, not the whole database
```

```bash
psql -h <host> -U <username> -d <database> -f backup.sql                     # restore a plain-SQL dump
pg_restore -h <host> -U <username> -d <database> backup.dump                   # restore a custom-format dump
...
pg_restore -h <host> -U <username> -d <database> -t <table> backup.dump              # restore just one table out of a full dump
```
...
pg_basebackup -h <host> -U <username> -D /backup/path -Ft -z -P      # same, but tarred and gzip-compressed output instead of a plain directory
```
