...
```bash
set -x   # turn tracing on mid-script — enable set -x only where the suspicious code actually starts
set +x   # turn it back off — brackets tracing to only the suspicious block instead of flooding the whole run
trap 'echo "failed at line $LINENO"' ERR   # know exactly where a script died — a trap on ERR reports the line number instead of leaving you guessing
PS4='+ $(date "+%T") ${BASH_SOURCE}:${LINENO}: '   # timestamped trace output when combined with set -x — shows how long each step actually took, not just what ran
```
