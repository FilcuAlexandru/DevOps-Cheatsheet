...
```bash
ss -tulnp | grep <port>          # find what's already bound to the port
kill <pid_from_above>              # or reconfigure one of the two services to use a different port
```

**DNS resolution suddenly fails:**
```bash
cat /etc/resolv.conf                 # confirm nameservers are actually configured
dig @8.8.8.8 <hostname>                # bypass the local resolver entirely — if this works, the problem is local DNS config, not the network
systemd-resolve --status                # on systemd-resolved hosts, shows which resolver is actually active per interface
```
