...
sar -q 1 5                     # historical load average + run queue length, needs sysstat
dstat                           # combined CPU/disk/net/memory view, one screen — a friendlier vmstat+iostat+ifstat
htop                             # interactive top with per-core bars, tree view, easier mouse-driven sorting
```
...
iotop -o                                       # like top but for disk I/O, -o hides processes doing zero I/O
```
