...
```bash
du -sh --max-depth=1 / 2>/dev/null | sort -rh | head   # which top-level directory is actually growing — narrows a full-disk hunt before you go file by file
...
kill -9 <parent_pid>                    # killing the parent usually clears it — if the parent is PID 1, only a reboot fixes it
```

**System killed my process (OOM):**
```bash
dmesg -T | grep -i "out of memory"   # confirm it was actually the OOM killer, not a crash
journalctl -k | grep -i oom            # OOM killer's own kernel log entries — works on any systemd distro, not just Debian/Ubuntu-specific /var/log/syslog
```
