...
def tooltipify_code(html_text):
    def repl_code(m):
        ...
        for line in lines:
            ...
            if cm:
                cmd_part, _, comment = cm.groups()
                tip = make_tip(comment)
                if tip:
                    tip_esc = html.escape(tip, quote=True)
                    comment_esc = html.escape(comment.strip())
                    out_lines.append(f'<span class="c"># {comment_esc}</span><span class="cmdline" tabindex="0" data-tip="{tip_esc}">{cmd_part}</span>')
                    continue
            out_lines.append(f'<span class="cmdline">{line}</span>')
        return open_tag + ''.join(out_lines) + close_tag
...
def render_page(top_slug, top_title, sub_slug=None, sub_title=None, children=None):
    ...
    if children:
        ...
        for cslug, ctitle in children:
            cards.append(f'<div class="card"><a href="./{cslug}/index.html">{ctitle}</a></div>')
...
# ---- grouped root landing page ----
def topic_status(slug, children):
    md_content = gather_content(slug)
    empty = (not md_content.strip()) or ('to fill in' in md_content.lower())
    if children:
        return f'{len(children)} sub-sections', False
    return ('not filled in yet', True) if empty else ('ready', False)
...
for group_name, slugs in GROUPS:
    ...
    for slug in slugs:
        ...
        desc, empty = topic_status(slug, children)
        cls = 'card empty' if empty else 'card'
        cards.append(f'<div class="{cls}"><a href="./{slug}/index.html">{title}</a><div class="desc">{desc}</div></div>')
    group_html.append(f'<div class="label" style="margin:28px 0 8px;">{group_name.upper()}</div><div class="grid">{"".join(cards)}</div>')
