...
argocd app delete <app_name>                                               # remove an app from ArgoCD (has its own flag for whether to also delete the underlying resources)
argocd proj list                                                             # ArgoCD projects — groupings that scope which repos/clusters/namespaces an app can use
argocd app logs <app_name> -f                                                  # stream logs for an app's pods directly through the ArgoCD CLI, no kubectl context switch needed
```
