...
omd status <site_name>                            # status for just one site, if running several on one host
cmk-passwd <username>                               # set/reset a Check_MK web UI user's password from the CLI
cmk --list-hosts                                      # every host currently configured in this site
cmk --version                                           # confirm the Check_MK version, relevant when checking plugin compatibility
```
...
cmk -O                                            # activate pending changes from the CLI (same effect as the "Activate changes" button in WATO)
cmk --backup <target_dir>                           # back up the whole site's configuration and data
cmk --checks=<check_name> -v <hostname>               # run just one specific check against a host, instead of the full discovery/check cycle
```
