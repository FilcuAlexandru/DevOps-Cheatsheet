...
**Service is down:**
```bash
systemctl status svc                    # is it even trying to run — and what's the last error before it stopped
journalctl -u svc -n 100 --no-pager      # recent logs, no pager to fight with — --no-pager dumps straight to the terminal instead of an interactive view
ss -tulnp | grep <port>                   # is anything actually listening — confirms the service bound to the port at all, before checking anything else
```

**High CPU:**
```bash
top -o %CPU                     # sorted live view, heaviest CPU consumer at the top — the fastest way to spot the actual culprit
ps -eo pid,cmd,%cpu --sort=-%cpu | head
strace -p <pid> -c              # is it doing real work or spinning on syscalls — distinguishes genuine CPU load from a process stuck in a loop
```

**Disk full:**
```bash
df -h                                    # usage per mounted filesystem — confirms which one is actually full, not just that disk usage is high somewhere
du -sh --max-depth=1 /suspect | sort -rh  # biggest subdirectories first, one level deep — no guessing where space went
lsof +D /path                             # check for deleted-but-still-open files — df/du disagreeing is the classic sign
```

**Network unreachable:**
```bash
ip route get <ip>          # which route/interface would be used — confirms traffic is even going out the path you expect
mtr <host>                   # where exactly it breaks — mtr shows which specific hop is dropping packets, not just that connectivity failed
...
keytool -list -v -keystore trust.jks | grep -A1 'Valid from'   # for Java truststores specifically — a different tool than openssl, since it's reading a Java keystore format
```

**Slow response times (no outright failure):**
```bash
curl -o /dev/null -s -w '%{time_total}s\n' https://endpoint    # confirm it's actually slow, not just perceived — a real number beats a feeling before you start debugging
vmstat 1 5                                                       # CPU-bound vs I/O-bound at a glance — points you toward the right next command instead of guessing
iostat -xz 1                                                     # disk %util — a common hidden cause
kubectl top pods --sort-by=cpu                                    # check if it's a specific pod hogging resources — narrows a cluster-wide slowdown to one culprit
```
...
```bash
kubectl rollout history deployment/myapp     # confirm what actually changed and when — a rollout history beats trusting memory of what was deployed
kubectl rollout undo deployment/myapp         # roll back first, investigate after — restore service before root-causing
git log --oneline -10                          # what changed in the code between the working and broken version — the actual diff, not a guess at what might have broken it
kubectl diff -f manifest.yaml                   # if it's a config/manifest change, see exactly what's different — confirms the real diff instead of assuming what changed
```
