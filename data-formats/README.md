...
```bash
jq '.items[] | select(.status=="Running") | .name' pods.json   # filter + extract in one line
...
yq -i '.spec.replicas = 3' deployment.yaml                           # updates one value inside a YAML file directly, without hand-editing it
python3 -c "import yaml,sys; yaml.safe_load(open(sys.argv[1]))" file.yaml   # validate YAML syntax with zero extra tools
```
...
yq -o=json '.' config.yaml | jq '.'                                          # convert YAML straight to JSON in one pipeline, then process it with jq's full feature set
```
...
cat -A file.json | head -3                 # look for invisible characters (a BOM, CRLF line endings) that break parsing but don't show up in a normal editor
```
